...
const ProfileCard = ({ userId, name, age, location, profession, company, facebookUrl, instagramUrl, linkedInUrl }) => {
        ...
        <div>
          ...
          >
            ...
            <div className="d-flex flex-column justify-content-center p-3 bg-dark bg-opacity-75" style={{ position: "absolute", bottom: "0", left: "0", width: "100%" }}>
              ...
              <p className="card-text mb-1"><img src="../placeholder.png" width={"20px"} height={"20px"} style={{ margin: "5px" }} alt="location" />{location}</p>
              <p className="card-text"><img src="../office.png" width={"20px"} height={"20px"} style={{ margin: "5px" }} alt="profession" />{profession} at {company}</p>
            ...
            <div className="d-flex justify-content-center" style={{ position: "absolute", bottom: "10px", width: "100%" }}>
              {preloadedImages.map((_, index) => (
                <div key={index} onClick={() => goToSlide(index)} style={{ height: "10px", width: "10px", margin: "0 5px", backgroundColor: currentIndex === index ? "white" : "gray", borderRadius: "50%", cursor: "pointer" }}></div>
              ))}
            </div>
          </div>
        </div>
